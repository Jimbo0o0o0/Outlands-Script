...
def save_items_data(
    ...
    try:
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            ...
            gold_amount = sum(
                item["amount"] for item in merged_items if item.get("id") == "9999999999"
            )
            doubblon_amount = sum(
                item["amount"] for item in merged_items if item.get("id") == "8888888888"
            )
            total_amount = sum(item["amount"] for item in merged_items) - gold_amount - doubblon_amount
            f.write(f"Total amount of items (excluding gold and doubloons): {total_amount}\n\n")

            for item in merged_items:
                f.write(f"{item['description']}, Amount: {item['amount']}\n")
...
def display_inventory_data(
    ...
    """Print a console summary (uses grouped unidentified items)."""
    print(f"Name: {name}")
    ...
    total_amount = sum(item["amount"] for item in merged_items) - gold_amount - doubblon_amount
    print(f"Total amount of items (excluding gold and doubloons): {total_amount}\n")

    for item in merged_items:
        print(f"{item['description']}, Amount: {item['amount']}")
